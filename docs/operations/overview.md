...
| Embedded Script jobs | Reference for the Embedded Script job type, which runs an inline script defined directly in OpCon without requiring a pre-deployed script file on the agent machine. |
...
Log in as root, change to the agent's root directory, and run `lsam<SAM_Socket> start`. For example: `cd /usr/local/lsam/; bin/lsam3100 start`. Refer to [Operating the agent](./operating-the-lsam.md) for the full procedure including how to verify that all required processes started.
...
Run `cat LSAM_output_<SAM_socket>` to view terminal output from the failed startup, then check the `logfile` and `errfile` in the `SMA_LOG_DIRECTORY` path for error messages. Common causes are missing SSL libraries (see [Installation Requirements](../installation/requirements.md)) and port conflicts (`EADDRINUSE`). Refer to [Troubleshooting](../reference/troubleshooting.md) for a full error symbol reference.
