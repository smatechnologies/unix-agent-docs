...
1. Installation of the agent for UNIX requires an installer with root privileges. As long as your account has a UID of ```0```, it does not matter what the account name is. You must have root access because the agent must do user impersonation (by calling ```setuid ()``` and ```setgid()``` ).
    * Installers without root privileges may be able to perform a sudo installation. For information, refer to [Performing a Sudo Installation](../installation/preferred-installation.md#performing-a-sudo-installation).
...
* The agent root directory is a link to the agent installation directory (e.g., ```/usr/local/lsam/```). This is the location used to access the agent. For example, the directory ```/usr/local/lsam/``` for agent version 5.20 would be linked to ```/usr/local/lsam-5.20```. This symbolic link allows access to the agent to stay the same through upgrades. This access is meant for system-level use (e.g., an entry in "```/etc/init.d```" used to auto-start the agent at boot-up). Access to agent utilities from within user jobs is via environment variable ```$SMA_BINDIR```, which gets defined during agent installation.
