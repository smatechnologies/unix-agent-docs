...
The ```<path>``` points to the agent "```bin/```" directory (e.g., "```/usr/local/lsam/bin```").
...
Assuming the ```captureSTDOUT``` file is in the agent ```/bin``` directory, the Start Image and Parameters on the UNIX Details screen (in the Enterprise Manager's Job Master) for the above job would contain the following:
