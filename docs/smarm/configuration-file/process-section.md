...
description: "Reference for the SMA_RM PROCESS section, which configures process monitoring conditions — including IGNORE, MUST_RUN, MUST_NOT_RUN, and CPU_CHECK — along with alarm levels and event variables."
tags:
  - Reference
...
- Configure a `<process>` section with `<condition>CPU_CHECK</condition>` when you need to detect processes that are consuming an inordinate amount of CPU time, using `<C_alarm>` and `<T_alarm>` thresholds to tune the sensitivity.
- Configure a `<process>` section with `<condition>IGNORE</condition>` before other `<process>` sections when certain processes — for example, all root processes — should be excluded from existence checks and CPU-hog detection but still counted in the total process count.
...
3. If the average of all ten "C" values meet or exceed a MAX ```<C_alarm>```, or if a MIN ```<C_alarm>``` is greater than the average, the process is considered to be a CPU-hog. (You can use a MIN value to catch a process that has quit working.)

4. If the new T value minus the oldest, as a percentage of the elapsed clock time, is greater than or equal to a MAX ```<T_alarm>```, or if a MIN ```<T_alarm>``` is greater than the difference in T values, the process is considered to be a CPU-hog. (You can use a MIN value to catch a process that has quit working.) Thus, if the scan interval is 1 second and ```<T_alarm>``` is 30, a process is considered to be a CPU-hog if it uses at least 3 seconds of CPU time within any 10-second period. If the scan interval were set to every 5 seconds and ```<T_alarm>``` to 20, then a process would be considered a CPU-hog if in any 50-second interval (10 scan cycles) it used 5 x 10 x 20% = 10 seconds of CPU time.
...
Within each SMA_RM scan cycle, any process with a UID of 'root' will be ignored. Next, a check will be made for existence of at least one process with 'data' within its name (CMD). Next, a check will be made to see if any processes from the "/usr/games" directory are running. And finally, with the added exclusion of any processes from the "/usr/games" directory, all other processes within the system will be inspected to see if they are CPU-hogs.

## Examples

The following example configures two process monitoring sections. The first ignores all processes owned by the `root` user so they are excluded from CPU-hog detection. The second alarms if a process with `data` in its name is not running and sends a console event to SAM, then sends a separate event when the process is detected again.
