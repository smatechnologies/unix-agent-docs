...
Scripts using the sma_status utility can use the 20-character message area to display any desired text; otherwise, the PID displays in the message area. For further information, refer to the example that shows a script using the sma_status utility on [sma_status](../../operations/utilities/sma-status.md).
...
The [x] multiply-defined message text applies only to jobs based on the SMA Technologies-supplied template job script discussed in the Source - Unix Agent Details DOC.
...
The Invalid job step [x] applies only to jobs based on the SMA Technologies-supplied template job script discussed in the Source - Unix Agent Details DOC.
...
The Infinite loop detected applies only to jobs based on the SMA Technologies-supplied template job script discussed in the Source - Unix Agent Details DOC.
...
### Received order to stop agent health monitor	
